...
dm.mds <- DiffusionMap(mds.df, vars=1:6)
...
plot(dm.mds,c(2,3,1),pch = 20,legend_main = 'Timepoint', col_by='sample')
...
plot(dm,c(1,3,2),pch = 20,legend_main = 'Timepoint', col_by='sample')
...
#### PSEUDOTIME VS TIME ####
ggplot(df,aes(x=as.numeric(factor(time, levels=c('2h','8h','16h','24h','48h','72h', '0h'))), y=as.numeric(-pseudotime))) +  
  geom_point() +
...
################## TEMPORAL DIFFERENTIAL EXPRESSION DESTINY ###################
Y <- counts
t <- DPT(dm.mds)$dpt
plot(t, 1L, 1:6, col_by='branch', pch=21)
...
write.csv(top100.p, paste0(root, '/new_tables/FINAL_WORKFLOW/top100_psuedotime_gens_destiny.csv'))


## common DEG between pseudotime and time by pval ##
deg <- readRDS(paste0(root, '/new_SCE_objects/FINAL_WORKFLOW/deg_cellthr2.rds'))
top100.t <- deg %>% arrange(pval_adj) %>% slice_head(n=100) ## deg between time
intersect(top100.p$gene_name, top100.t$gene) ## 21/500 common DEG  ## 4% 
heatdata <- as.matrix(counts[top100.p$gene_name, order(t, na.last = NA)])
heatclus <- s$sample[order(t, na.last = NA)]
...
png(paste0(root, '/new_visualisations/FINAL_WORKFLOW/destiny_pseudotime.png'))
dev.off()
...
###### plot slingshot pseudotime vs real time ######
df <- data.frame(pseudotime1=sce$slingPseudotime_1, pseudotime2=sce$slingPseudotime_2, 
  sample=sce$sample) 
df <- df %>% dplyr::filter(!is.na(pseudotime1))
df <- df %>% dplyr::filter(sample!='72h')
p2 <- ggplot(df,aes(x=as.numeric(factor(sample, levels=c('2h','8h','16h','24h','48h', '72h', '0h'))), 
  y=as.numeric(pseudotime1))) +  
  geom_point() +
  geom_smooth(method='loess', se=F, color=cc[2]) +
  scale_x_continuous(name='Time', breaks=c(1,2,3,4,5,6,7), labels=c('2h','8hr','16h','24h', '48h','72h', '0h')) + 
  labs(y='Pseudotime') + ggtitle('pseudotime trajectory 2')
cowplot::plot_grid(p1,p2)
ggsave(paste0(root, '/new_visualisations/FINAL_WORKFLOW/pseudotime_loessmds6_slingshotcomb.png'), width=10, height=7)
df$pseudorank <- rank(df$pseudotime1)
ggplot(as.data.frame(df), 
  aes(x = pseudorank, 
  y = factor(sample, levels=c('2h','8h','16h','24h','48h','72h', '0h')), colour = sample)) +
  geom_quasirandom(groupOnX = FALSE, size=2) +
  scale_color_tableau()+
  ...
  scale_colour_manual(values=c(cc[1], cc[2], cc[3], cc[4], cc[5], cc[6], cc[7]), 
  guide = guide_legend(override.aes = list(size = 5), title='Timepoint')) + 
  ggtitle('pseudotime trajectory 1')
ggsave(paste0(root, '/new_visualisations/FINAL_WORKFLOW/pseudotime_slingshotP2.png'), width=7, height=7)


############ DIFFERENTIAL EXPRESSION SLINGSHOT PSEUDOTIME ##############
...
## venn diagram between DE genes across time and pseudotime ##
t.genes <- top100.t %>% dplyr::select(gene) %>% unlist()
p.genes <- names(top.genes) %>% unlist()
venn.diagram(
  x = list(p.genes, t.genes),
  category.names = c('Pseudotime DE genes','DE genes across time'),
  filename = paste0(root, '/new_visualisations/FINAL_WORKFLOW/common_DE_pseudotime_time.png'),
  output=TRUE,
...
Heatmap(heatdata, top_annotation = ha.anno, 
     ...
     cluster_columns = F, 
     show_column_names = F) +
  rowAnnotation(link=anno_mark(at = which(rownames(heatdata) %in% common),
  labels = (rownames(heatdata)[rownames(heatdata) %in% common]), 
...
## save SCE object ##
saveRDS(sce, paste0(root, '/new_SCE_objects/FINAL_WORKFLOW/trajectory_sce.rds'))
